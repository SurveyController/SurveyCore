...
// UserAgentProfile is one selectable user-agent preset.
...
// DefaultRandomUARatios returns a copy of the default category weights.
func DefaultRandomUARatios() map[string]int {
	return cloneDefaultRandomUARatios()
}

// SelectUserAgentFromRatios samples a preset from category weights.
...
// RuntimeUserAgent returns a sampled user-agent or an empty string when disabled.
...
// SampleAnswerDurationSeconds samples an answer duration from a normalized range.
...
// SampleSubmitIntervalSeconds samples an inclusive submit delay in seconds.
