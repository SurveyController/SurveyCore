...
// WeightTable stores either option weights or per-row matrix weights.
type WeightTable struct {
	Options []float64   `json:"options,omitempty"`
	Rows    [][]float64 `json:"rows,omitempty"`
}

// QuestionStrategy configures answer generation for one question.
...
// ConsistencyRule applies a target answer action when a condition matches.
...
// AnswerPlan groups cross-question rules, dimensions, and question strategies.
