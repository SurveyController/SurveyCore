...
// NormalizeRunRequestPayload normalizes supported legacy configuration fields.
...
func migrateLegacyPayload(raw map[string]any) (model.RunRequest, error) {
	for key := range raw {
		if !legacyFields[key] {
			return model.RunRequest{}, fmt.Errorf("该配置文件损坏：配置包含不支持的字段（%s）", key)
		}
	}
	...
	if questions, err := decodeQuestions(raw["questions_info"]); err != nil {
		return model.RunRequest{}, fmt.Errorf("旧配置题目快照无效：%w", err)
	} else {
