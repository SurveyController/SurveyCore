...
func TestConfigDocumentV2RoundTrip(t *testing.T) {
	document := ConfigDocument{
		...
		Survey: SurveyDocument{
			URL: "https://wj.qq.com/s2/123/hash/", Provider: model.ProviderQQ, Title: "腾讯测试",
			Definition: model.SurveyDefinition{Provider: model.ProviderQQ, Title: "腾讯测试"},
		},
		...
		Answers:       model.AnswerPlan{Dimensions: []string{"服务", "价格"}},
		ReverseFill:   model.ReverseFillPlan{Enabled: true, SourcePath: "D:/demo.xlsx", Format: ReverseFillFormatWJXSequence, StartRow: 3, Threads: 2},
	...
	if restored.SchemaVersion != ConfigSchemaVersion || restored.Survey.Provider != model.ProviderQQ || restored.Execution.Target != 12 || !restored.Network.RandomProxyEnabled {
...
func TestRepositoryExampleUsesV2Schema(t *testing.T) {
	path := filepath.Join("..", "..", "..", "configs", "example.json")
	document, err := LoadDocument(path, true)
	if err != nil {
		t.Fatal(err)
	}
	if document.SchemaVersion != ConfigSchemaVersion {
		t.Fatalf("schema version = %d, want %d", document.SchemaVersion, ConfigSchemaVersion)
	}
	if document.Survey.Provider != model.ProviderWJX || document.Execution.Target != 10 {
		t.Fatalf("example document = %#v", document)
	}
}
...
func TestDeserializeLegacyPayloadMigratesToV2Model(t *testing.T) {
	cfg, err := DeserializeRunRequest(map[string]any{
		...
		"survey_provider":        model.ProviderWJX, "reverse_fill_threads": "0",
...
func TestLegacyV1AllFieldsMigrateIntoV2Sections(t *testing.T) {
	payload := map[string]any{
		...
		"survey_title":             "旧问卷",
		"survey_provider":          model.ProviderQQ,
		"target":                   12,
		"threads":                  4,
		...
		"questions_info":           []any{map[string]any{"num": 1, "title": "满意度", "type_code": "3", "options": 2, "provider": model.ProviderQQ, "option_texts": []any{"否", "是"}}},
	}
	...
	if document.SchemaVersion != ConfigSchemaVersion || document.Survey.Title != "旧问卷" || document.Survey.Provider != model.ProviderQQ || len(document.Survey.Definition.Questions) != 1 {
...
func TestDeserializeV2RejectsUnknownFields(t *testing.T) {
	_, err := DeserializeConfigDocument(map[string]any{
		"schemaVersion": ConfigSchemaVersion,
		"survey":        map[string]any{"url": "https://example.test", "provider": model.ProviderWJX, "title": "", "definition": map[string]any{}},
...
func TestLoadSaveConfigWritesV2WithoutLegacyAIKey(t *testing.T) {
	...
	cfg := model.RunRequest{SurveySource: model.SurveySource{URL: "https://example.test", Provider: model.ProviderWJX}, ExecutionPlan: modelExecution(9, 1)}
...
func TestSaveDocumentReplaceFailurePreservesOriginalFile(t *testing.T) {
	...
	document := ConfigDocumentFromRunRequest(model.RunRequest{
		SurveySource: model.SurveySource{URL: "https://example.test", Provider: model.ProviderWJX},
	})
...
func modelExecution(target, threads int) model.ExecutionPlan {
	return model.ExecutionPlan{Target: target, Threads: threads, AnswerDuration: [2]int{60, 120}, FailStop: true}
}
