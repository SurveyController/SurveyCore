...
func createSchema(db *sql.DB) error {
	_, err := db.Exec(`
		...
		CREATE TABLE IF NOT EXISTS v1_task_logs (
		...
		CREATE INDEX IF NOT EXISTS idx_v1_task_logs_task_id_id
			ON v1_task_logs(task_id, id);

		CREATE TABLE IF NOT EXISTS v1_proxy_session (
			id INTEGER PRIMARY KEY CHECK (id = 1),
			session_json TEXT NOT NULL,
			updated_at TEXT NOT NULL
		);

		INSERT OR IGNORE INTO v1_schema_migrations(version, applied_at)
...
func (s *Store) SaveTask(task *Task) error {
	...
	_, err = s.database().Exec(`
		INSERT INTO v1_tasks(id, status, record_json, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?)
...
func (s *Store) AppendLog(taskID string, entry TaskLog) error {
	...
	_, err = s.database().Exec(`
		INSERT INTO v1_task_logs(task_id, timestamp, level, message, fields_json, event_json)
		VALUES (?, ?, ?, ?, ?, ?)
...
func (s *Store) LoadTasks() ([]*Task, []error) {
	rows, err := s.database().Query(`SELECT record_json FROM v1_tasks ORDER BY created_at DESC`)
	if err != nil {
...
func (s *Store) database() *sql.DB {
	...
	defer s.mu.Unlock()
	if s.db == nil {
		panic("service.Store.Init 必须在读写前调用")
	}
	return s.db
}
...
func (s *Store) SaveSession(_ context.Context, session proxycore.RandomIPSession) error {
	...
	_, err = s.database().Exec(`
		INSERT INTO v1_proxy_session(id, session_json, updated_at)
		VALUES (1, ?, ?)
		ON CONFLICT(id) DO UPDATE SET
